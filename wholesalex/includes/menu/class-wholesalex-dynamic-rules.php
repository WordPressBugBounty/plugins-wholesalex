...
class WHOLESALEX_Dynamic_Rules {
	 ...
	 */
	function wholesalex_handle_dynamic_rule_action(\WP_REST_Request $request)
	{
		$action = $request->get_param('action');
		$ruleIds = $request->get_param('ruleIds');
		$rules = get_option('__wholesalex_dynamic_rules', []);
		$dynamic_message = '';
		if (!is_array($rules) || empty($ruleIds)) {
		...
		return array('success' => true, 'message' => 'Bulk action ' . $dynamic_message . ' completed successfully');
	}
	...
	public static function get_shipping_country()
	{
		...
		$result = array_map(function ($name, $value) {
			return array('name' => $name, 'value' => $value); }, $countryList, array_keys($countryList));
	...
	public function price_table_layout_css()
	{
		...
		<style>
			...
			.wsx-price-table-container {
				width: max-content;
				border: 1px solid
					<?php echo $table_border_color ?>
				;
				margin-bottom: 40px;
				font-size:
					<?php echo $table_font_size ?>
					px;
				color:
					<?php echo $table_text_color ?>
				;
				border-radius:
					<?php echo esc_attr(($is_table_radius == 'no') ? '0' : '8px'); ?>
				;
			}
			...
			.wsx-price-table-header {
				...
				;
				color:
					<?php echo $table_title_text_color ?>
				;
				background-color:
					<?php echo $table_title_bg_color ?>
				;
				border-radius:
					<?php echo esc_attr(($is_table_radius == 'no') ? '0' : '8px 8px 0 0'); ?>
				;
			}
			...
			.wsx-price-table-row.active {
				color:
					<?php echo $table_active_text_color ?>
				;
				background-color:
					<?php echo $table_active_bg_color ?>
				;
			}
			...
			.layout-vertical .wsx-price-table-header {
				...
				border-bottom: 0;
				border-radius:
					<?php echo esc_attr(($is_table_radius == 'no') ? '0' : '8px 0 0 8px'); ?>
				;
			}
			...
			.wsx-price-classical-tag {
				padding: 0 6px;
				font-size:
					<?php echo $table_font_size - 4 ?>
					px;
				color:
					<?php echo $table_discount_text_color ?>
				;
				background-color:
					<?php echo $table_discount_bg_color ?>
				;
				border-radius:
					<?php echo esc_attr(($is_table_radius == 'no') ? '0' : '2px'); ?>
				;
			}
			...
			.layout-vertical .wsx-price-classical-item {
				padding: 8px 12px;
				border-radius:
					<?php echo esc_attr(($is_table_radius == 'no') ? '0' : '4px'); ?>
				;
			}
	...
	public function wholesalex_price_table_generator($regular_price, $product)
	{
		...
		if (wholesalex()->get_single_product_setting($product_id, '_settings_tire_price_product_layout')) {
			...
			if ($product_tire_layout == 'table_style' && $is_vertical_layout == 'no' || $tier_layout_product_compatibility == 'layout_four' || $tier_layout_product_compatibility == 'layout_one') {
				$tier_layout = 'layout_one';
			} elseif ($product_tire_layout == 'table_style' && $is_vertical_layout == 'yes' || $tier_layout_product_compatibility == 'layout_six') {
				$tier_layout = 'layout_six';
			} elseif ($product_tire_layout == 'classic_style' && $is_vertical_layout == 'no' || $tier_layout_product_compatibility == 'layout_five' || $tier_layout_product_compatibility == 'layout_two' || $tier_layout_product_compatibility == 'layout_seven') {
				$tier_layout = 'layout_two';
			} elseif ($product_tire_layout == 'classic_style' && $is_vertical_layout == 'yes' || $tier_layout_product_compatibility == 'layout_three' || $tier_layout_product_compatibility == 'layout_eight') {
			...
			if ($tier_Table_style_layout == 'table_style' && $is_vertical_layout == 'no' || $tier_layout_compatibility == 'layout_four' || $tier_layout_compatibility == 'layout_one') {
				$tier_layout = 'layout_one';
			} elseif ($tier_Table_style_layout == 'table_style' && $is_vertical_layout == 'yes' || $tier_layout_compatibility == 'layout_six') {
				$tier_layout = 'layout_six';
			} elseif ($tier_Table_style_layout == 'classic_style' && $is_vertical_layout == 'no' || $tier_layout_compatibility == 'layout_five' || $tier_layout_compatibility == 'layout_two' || $tier_layout_compatibility == 'layout_seven') {
				$tier_layout = 'layout_two';
			} elseif ($tier_Table_style_layout == 'classic_style' && $is_vertical_layout == 'yes' || $tier_layout_compatibility == 'layout_three' || $tier_layout_compatibility == 'layout_eight') {
				$tier_layout = 'layout_three';
			}
		}
		...
		switch ($tier_layout) {
			...
			case 'layout_six':
				?>
				<div class="wsx-price-container-title"><?php echo $table_label; ?></div>
				<div class="wsx-price-table-container <?php echo $tier_layout == 'layout_six' ? 'layout-vertical' : ''; ?>">
					<div class="wsx-price-table-header">
						<div class="wsx-price-table-row">
							<?php
							foreach ($table_column_priority as $column) {
								if (isset($column['status']) && ($column['status'] == 'yes' || $column['status'])) {
									?>
			...
			case 'layout_three':
				...
				?>
				<div class="wsx-price-container-title"><?php echo $table_label; ?></div>
				<div class="wsx-price-classical-container <?php echo $tier_layout == 'layout_three' ? 'layout-vertical' : ''; ?>">
					<?php
					$__tier_size = count($quantity_prices);
					for ($i = 0; $i < $__tier_size; $i++) {
						$__current_tier = $quantity_prices[$i];
						...
						<div
							...
							<div class="wsx-price-classical-content">
								<div class="wsx-price-classical-price">
									<?php
									if ($__show_discount_amount && $tier_layout != 'layout_three') {
								...
								<?php if (esc_attr((isset($__current_tier['_id']) && $__current_tier['_id'] === $active_tier)) && $tier_layout != 'layout_three') { ?>
					...
					<div class="wsx-sp-cart-discounts">
						...
						<div class="wsx-sp-discounts-cards wsx-p-4 wsx-br-sm wsx-mt-8 wsx-bg-promotion">
							...
							foreach ($cart_discounts as $cd) {
								...
								$desc = '<div class="wsx-font-14 wsx-mb-4">' . wholesalex()->get_setting('cart_discount_promo_sp_desc_text', __('After adding to the cart', 'wholesalex')) . ' </div>';
								$conditions = 'yes' === wholesalex()->get_setting('show_discount_conditions_on_sp', 'no') && isset($cd['conditions']['tiers']) ? $this->generate_rule_conditions_markup($cd['conditions']['tiers']) : '';
								$validity = '';
								if ('yes' === wholesalex()->get_setting('show_discounts_validity_text_on_sp', 'no')) {
					...
					<div class="wsx-bxgy-free-item-meta">
						...
						if ($free_item_quantity > 1) {
							...
							<div class="wsx-bxgy-free-item-qty">
								<?php echo sprintf(__('Quantity: %s', 'wholesalex'), $free_item_quantity); //phpcs:ignore ?> </div>
						<?php } //phpcs:ignore ?>
						<div class="wsx-bxgy-free-item-price">
							<?php echo wc_price($product->get_price('edit') * esc_html($free_item_quantity));//phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped  ?>
						</div>
					</div>
				...
				<?php if ($badge_style === 'style_one' || $badge_style === '' || empty($badge_style)) { ?>
					.wholesalex-bogo-badge-<?php echo esc_attr($badge_dynamic_rule['id']); ?>::before {
						border-right: 15px solid
							<?php echo esc_attr($badge_label_bg_color); ?>
						;
					}
					...
					.wholesalex-bogo-badge-<?php echo esc_attr($badge_dynamic_rule['id']); ?> {
						right:
							<?php echo $badge_position == 'left' ? 'auto' : '0px'; ?>
						;
						left:
							<?php echo $badge_position == 'left' ? '0px' : 'auto'; ?>
						;
						margin-left:
							<?php echo $badge_position == 'left' ? '14px' : '0'; ?>
						;
					}
